...
int lre_8(unsigned char array[], unsigned int * array_length){
	...
	unsigned char escape = 1; //need to be a parameter
	...
	for (j = 0; j < symbols_8_length; j++)
	{
		...
		if (symbols_occ_8[j]>3){
			...
			if (symbols_occ_8[j]<=symbols_occ_limit){
				array[i]=escape;
...
int lre_32(unsigned char array[], unsigned int * array_length){
	int i, j, o;
	int symbols_occ_limit=255; //
	unsigned char escape = 1; //need to be a parameter
...
static int parseXmlFile(char * xmlfilename, char * outfilename) {
	...
	if (tot == 0){
		printf("There are not bitfiles to process.\n");
		return(-1);
	}else{
		printf("There are %d bitfile/s to process.\n", tot);
		fp = fopen(outfilename, "a+");
		if (NULL == fp){
			printf("File opening failed.\n");
			return(-1);
...
int main(int argc, char **argv) {
	if (argc == 3){
		...
		printf("Start convbitfile - SPLASH!.\n");
		char * xmlfilename;
		char * outfilename;
		xmlfilename = argv[1];
		outfilename = argv[2];
